import { world, system, ItemStack, ItemType } from "@minecraft/server";
...
//   const touketu = world.getComponent("wj:touketu");
//   touketu.registerCustomComponent(
//     "wj:touketu",
//     {
//       onUse({ source }) {
...
//     resolve(world.getDimension("overworld"));
//   });
// });
...
function clear_mapinfo() {
  ...
  objective.removeParticipant("§7Map: §eDELTARUNE");
}

function set_map(message, sourceEntity) {
  let objective = world.scoreboard.getObjective("info");
  if (message <= 7 && message >= 0) {
    ...
    switch (world.getDynamicProperty("map")) {
      ...
      case (6): // New Castle
        ...
        break;
      case (7): // DELTARUNE
        sourceEntity.runCommand('tellraw @a {"rawtext":[{"text":"§7[§e 設定 §7] §7MAPを §e§lDELTARUNE §r§7にしました"}]}');
        clear_mapinfo();
        objective.setScore("§7Map: §eDELTARUNE", 99);
        break;
    }
  } else {
    sourceEntity.sendMessage("Error : Message's Range is 0 ~ 7");
  }
}
...
function choosemap() {
  ...
  switch (Number(world.getDynamicProperty("map"))) {
    ...
    case (6): // New Castle
      ...
      break;
    case (7): // DELTARUNE
      sys.runCommand("tp @a[tag=sanka] -223 -60 264");
      sys.runCommand("spawnpoint @a[tag=sanka] -223 -60 264");
      break;
  }
